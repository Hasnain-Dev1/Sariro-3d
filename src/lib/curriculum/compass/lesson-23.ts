...
export const lesson23: StructuredLesson = {
  ...
  quiz: [
    ...
    { id: 'c23q3', kind: 'code_reading', prompt: 'What does run_step_safely() return if run_agent_loop() raises an exception?', options: ['Raises the exception further', '(False, an error message describing the failure)', '(True, None)', 'Crashes the program'], answerIndex: 1, explanation: 'The try/except catches the exception and returns a tuple signaling failure instead of propagating it.' },
    ...
    { id: 'c23q9', kind: 'application', prompt: 'Why is graceful degradation preferable to crashing the whole plan on one failed step?', options: ['It isn’t preferable', 'Other steps may still succeed and deliver real value, even if one part couldn’t be completed', 'Crashing is always better', 'It has no benefit'], answerIndex: 1, explanation: 'Delivering partial, honestly-labeled value beats an all-or-nothing failure.' },

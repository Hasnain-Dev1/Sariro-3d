...
export const lesson08: StructuredLesson = {
  ...
  subtitle: "Actually run the calculator and send its result back to the model for a real, final answer.",

  concept: {
    durationMin: 15,
    summary:
      "Learn how to execute a requested tool, send its result back to the model in a tool-role message, and get a final, grounded answer.",
    sections: [
      {
        heading: 'The full round trip',
        body:
          "Tool use is a CONVERSATION, not a single call: (1) you send a question with tools available, (2) the model replies with one or more tool_calls, (3) YOUR code runs the actual tool function, (4) you append the assistant's message plus a NEW tool-role message with that output, (5) the model reads the result and gives a final text answer. This is the agent loop from Lesson 1, now fully real.",
      },
      {
        heading: 'Executing the tool',
        body:
          "Each tool call gives you the tool's name and a JSON string of arguments (parsed with json.loads()) — you write a plain function that does the real work (here, evaluating a math expression) and call it based on which tool was requested.",
        code: {
          language: 'python',
          code:
      ...
      {
        heading: 'Sending the result back',
        body:
          "You build on the SAME messages list: append the model's own message (response.choices[0].message, which contains the tool_calls) directly, then append a new message with role \"tool\", a tool_call_id matching the call, and the tool's output as content. Sending this back lets the model finish its answer using REAL data.",
        code: {
          language: 'python',
          code:
      ...
      {
        heading: 'Why tool_call_id matters',
        body:
          "Every tool call has a unique id. Your tool-role message MUST reference that exact id via tool_call_id — this is how the model matches your result to the specific request it made, especially important once an agent can request MULTIPLE tools at once (a later lesson).",
      },
      {
    ...
    keyTerms: [
      { term: 'tool-role message', definition: "A message with role \"tool\" sending a tool's output back to the model, referencing its tool_call_id." },
      { term: 'tool_call_id', definition: "A unique id on a tool call, used to match a later tool-role message to the correct request." },
      { term: 'Round trip', definition: "The full exchange: question → tool call request → your execution → tool result message → final answer." },
      { term: 'Grounded answer', definition: "A reply based on REAL tool output, not the model's own (possibly wrong) internal guess." },
    ],
    commonMistakes: [
      "Forgetting to append the model's ORIGINAL message (with its tool_calls) to the messages list before appending the tool result.",
      "Sending a tool-role message with the wrong (or missing) tool_call_id, so the model can't match it to its request.",
    ...
    takeaways: [
      ...
      "Execute the tool yourself, then send its output back as a role: 'tool' message.",
      "Always append the model's original message AND match the tool_call_id exactly.",
  ...
  miniProject: {
    ...
    code: [
      {
        ...
        code:
          "from datetime import datetime\nfrom dotenv import load_dotenv\nfrom openai import OpenAI\n\nload_dotenv()\nclient = OpenAI()\nMODEL = \"gpt-4o-mini\"\n\nget_time_tool = {\n    \"type\": \"function\",\n    \"function\": {\n        \"name\": \"get_time\",\n        \"description\": \"Returns the current date and time.\",\n        \"parameters\": {\"type\": \"object\", \"properties\": {}, \"required\": []},\n    },\n}\n\ndef main():\n    question = \"What time is it right now?\"\n    messages = [{\"role\": \"user\", \"content\": question}]\n\n    first = client.chat.completions.create(\n        model=MODEL, max_tokens=200, tools=[get_time_tool], messages=messages,\n    )\n\n    message = first.choices[0].message\n    if not message.tool_calls:\n        print(\"No tool needed:\", message.content)\n        return\n\n    call = message.tool_calls[0]\n    tool_output = str(datetime.now())   # \"execute\" the tool for real\n\n    messages.append(message)\n    messages.append({\"role\": \"tool\", \"tool_call_id\": call.id, \"content\": tool_output})\n\n    follow_up = client.chat.completions.create(\n        model=MODEL, max_tokens=200, tools=[get_time_tool], messages=messages,\n    )\n\n    print(follow_up.choices[0].message.content)\n\nif __name__ == \"__main__\":\n    main()",
      },
    ],
    explanation:
      "The first call detects the need for get_time and returns a message with a tool_calls entry. datetime.now() IS the actual tool execution — a genuine real value, not simulated. Before the follow-up call, the SAME messages list gets extended: the model's own message (with its tool_calls) is appended unchanged, then a new role: 'tool' message carries the real time, matched to the exact tool_call_id. The model then reads that real value and answers with it directly — a genuinely grounded, correct final answer rather than a guess.",
    expectedOutput:
    ...
    learned: [
      ...
      "How to extend a messages list correctly for a tool round trip.",
      "Why matching tool_call_id is essential.",
  ...
  finalProject: {
    ...
    code: [
      ...
      {
        language: 'python',
        filename: 'main.py (replace the tool_calls handling in ask_compass)',
        code:
          "def ask_compass(question: str) -> str:\n    try:\n        messages = [\n            {\"role\": \"system\", \"content\": SYSTEM_PROMPT},\n            {\"role\": \"user\", \"content\": question},\n        ]\n\n        response = with_retry(lambda: client.chat.completions.create(\n            model=MODEL, max_tokens=400, temperature=0.2,\n            tools=TOOLS, messages=messages,\n        ))\n\n        message = response.choices[0].message\n        if message.tool_calls:\n            call = message.tool_calls[0]\n            args = json.loads(call.function.arguments)\n\n            if call.function.name == \"calculator\":\n                tool_output = run_calculator(args[\"expression\"])\n                print(f\"[tool] calculator({args['expression']!r}) = {tool_output}\")\n            else:\n                tool_output = \"Error: unknown tool.\"\n\n            messages.append(message)\n            messages.append({\"role\": \"tool\", \"tool_call_id\": call.id, \"content\": tool_output})\n\n            follow_up = with_retry(lambda: client.chat.completions.create(\n                model=MODEL, max_tokens=400, temperature=0.2,\n                tools=TOOLS, messages=messages,\n            ))\n            return follow_up.choices[0].message.content\n\n        return message.content\n    except Exception as err:\n        print(f\"[error] Compass could not get a response: {err}\")\n        return \"Sorry, I'm having trouble right now. Please try again in a moment.\"",
      },
    ],
    placement:
      "Add run_calculator near your TOOLS list. Replace the tool_calls branch inside ask_compass() (the placeholder from Lesson 7) with the full round-trip version above — everything else in the file (SYSTEM_PROMPT, with_retry, TOOLS) stays the same.",
    implementation:
      "The if call.function.name == 'calculator': check makes the code EXTENSIBLE — Lesson 9's homework tool and future tools just add another branch here. run_calculator is wrapped in try/except internally, so a malformed expression returns a clear error string INSTEAD of crashing — and that error string is itself sent back as the tool-role message content, letting the model explain the problem to the user naturally rather than your code needing special-case handling for it. The follow_up call reuses with_retry, so this second network round-trip is just as resilient as the first.",
    expectedResult:
  ...
  quiz: [
    { id: 'c8q1', kind: 'concept', prompt: 'What are the FIVE steps of a complete tool-use round trip?', options: ['Ask, wait, ask again', 'Send question -> tool call response -> execute tool -> send tool result message -> final answer', 'Login, query, logout, retry, done', 'Prompt, temperature, tokens, model, response'], answerIndex: 1, explanation: "This is the full cycle described in the concept section — a genuine multi-message exchange." },
    { id: 'c8q2', kind: 'code_reading', prompt: 'Why must the follow-up messages list include the model’s ORIGINAL message (with its tool_calls) before the tool-role message?', options: ['It’s optional and can be skipped', 'So the model has full context of what it asked for when it reads the result', 'It changes the API key', 'It’s only for logging'], answerIndex: 1, explanation: "Including the model's own prior turn preserves conversational context for interpreting the result correctly." },
    { id: 'c8q3', kind: 'concept', prompt: 'What is tool_call_id used for?', options: ['Rate limiting', 'Matching a tool-role result message to the specific tool call it answers', 'Billing', 'Model selection'], answerIndex: 1, explanation: "It's the identifier connecting a result back to the exact request that triggered it." },
    { id: 'c8q4', kind: 'debug', prompt: 'A follow-up call is sent WITHOUT the tool_call_id matching the original request. What happens?', options: ['It works exactly the same', 'The model can’t correctly match the result to its request, likely causing confusion or an error', 'It automatically retries', 'The API ignores tool_call_id entirely'], answerIndex: 1, explanation: "Correct matching is required for the model to interpret the tool result properly." },
    ...
    { id: 'c8q6', kind: 'output', prompt: 'What does the follow_up call ultimately return, once a tool result message has been sent?', options: ['Another tool call always', 'A final, grounded text answer using the real tool output', 'The same tool_calls list again', 'An empty response'], answerIndex: 1, explanation: "With the real result in hand, the model typically completes its answer as plain text." },
    { id: 'c8q7', kind: 'application', prompt: 'Why send the calculator’s ERROR string back as the tool result instead of handling the error separately in your own code?', options: ['You must always crash on tool errors', 'Letting the model see the error lets IT explain the problem to the user naturally, without special-case code', 'It’s not possible to send an error as a result', 'Error strings are invalid tool message content'], answerIndex: 1, explanation: "The model can incorporate the error into a natural-language explanation, avoiding duplicate error-handling logic." },
    ...
    { id: 'c8q9', kind: 'project', prompt: "Why does the follow_up call in Compass's final project reuse with_retry, just like the first call?", options: ['It’s redundant and unnecessary', 'The follow-up is ALSO a real network call that can transiently fail, so it deserves the same reliability', 'with_retry only works once per session', 'Retries are only needed for tool-requesting responses'], answerIndex: 1, explanation: "Every network call, including the follow-up, benefits from the same resilience pattern." },
  ...
  homework: {
    task:
      "Implement the get_word_count tool from Lesson 7's homework FOR REAL: add a run_word_count(text) function, wire it into the tool-call branch, and confirm Compass gives a correct, grounded word count.",
    requirements: [
      "Write def run_word_count(text: str) -> str that splits on whitespace and returns the count as a string.",
      "Add an elif branch for call.function.name == 'get_word_count' calling it.",
    ...
    previousHomeworkHint: {
      ...
      steps: [
        ...
        "Its parameters schema needs one required string property, e.g. text.",

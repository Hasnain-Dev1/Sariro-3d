...
export const lesson09: StructuredLesson = {
  ...
  concept: {
    ...
    sections: [
      ...
      },
      {
        ...
        body:
          "Instead of one if not message.tool_calls, wrap the whole exchange in a loop: keep calling the API, and EVERY time the reply comes back with tool_calls, execute each one, append the result, and call again — repeat until the model returns a message with NO tool_calls, meaning it's finished and message.content holds the final answer.",
        code: {
          language: 'python',
          code:
      ...
      },
      {
        ...
        body:
          "This loop — call, check, act, repeat until done — IS what people mean by 'an AI agent' in the broadest sense. Everything after this lesson (memory, planning, deployment) adds capability around this same core loop, not a different one. It's also worth noting this exact loop shape works unchanged whether the model behind client is from OpenAI, Groq, or Gemini — the openai package's chat.completions interface is what stays constant.",
      },
    ],
    keyTerms: [
      ...
      { term: 'message.tool_calls', definition: "A list on the model's reply of the tool calls it wants to make this turn; empty or None means it produced a final answer instead." },
    ],
    commonMistakes: [
      "Handling only one tool call and assuming that's always enough — some questions genuinely need several in sequence.",
      "Forgetting to append the assistant's message (with its tool_calls) into the messages list before continuing the loop.",
      ...
      "Forgetting tool_call.function.arguments is a raw JSON string and must be parsed with json.loads() before use.",
    ],
    takeaways: [
      "Some questions need MULTIPLE tool calls in sequence, not just one.",
      "A for loop (with a turn cap) repeats: call → check tool_calls → execute if needed → repeat.",
      "A dispatch function keeps multi-tool routing clean as tools grow.",
      "Always cap loop iterations to prevent runaway behaviour.",
      "This general loop IS the core shape of an agent — everything later builds around it, regardless of provider.",
    ],
  },
  ...
  finalProject: {
    ...
    fileLocation: "compass-agent/main.py (replace the tool-calling handling with a loop)",
    code: [
      {
        language: 'python',
    ...
    implementation:
      "messages now lives at the TOP of the function and accumulates every turn — the system prompt, the user question, the model's tool-calling replies, the tool results, the model again — exactly as in the mini-project. The for _ in range(MAX_TURNS) loop caps how many API calls one question can trigger, and the return AFTER the loop is the safety fallback if a question genuinely needs more steps than the cap allows (rare, but handled explicitly rather than looping forever). execute_tool replaces Lesson 8's inline if/else, so adding a third tool later means writing its implementation and adding ONE line to execute_tool — nothing else in ask_compass() changes. Note tool_call.function.arguments arrives as a JSON STRING, so json.loads() is required before the arguments can be used as a dict.",
    expectedResult:
  ...
  quiz: [
    { id: 'c9q1', kind: 'concept', prompt: 'Why does checking message.tool_calls just once sometimes fall short?', options: ['It never falls short', 'Some questions genuinely need MULTIPLE sequential tool calls, not just one', 'It’s a syntax error', 'tool_calls is unreliable'], answerIndex: 1, explanation: "A question can require chaining several tool results together, which a single check can't handle." },
    { id: 'c9q2', kind: 'code_reading', prompt: 'Why does the loop append the assistant’s message (with its tool_calls) into messages BEFORE checking again?', options: ['It’s unnecessary', 'So the full conversation history (including that turn) is available for the NEXT call in the loop', 'It deletes old messages', 'It only affects logging'], answerIndex: 1, explanation: "Each loop iteration needs the complete, accumulated conversation to reason correctly." },
    ...
    { id: 'c9q4', kind: 'debug', prompt: 'A student’s loop never terminates on a normal question. Likely bug?', options: ['The turn cap is too low', 'The exit check is wrong, so it never recognises that message.tool_calls is empty', 'Too many tools are defined', 'temperature is too low'], answerIndex: 1, explanation: "If the exit condition (not message.tool_calls) is broken, the loop can't recognise when the model is actually done." },
    ...
    { id: 'c9q7', kind: 'output', prompt: 'For a question needing 2 tool calls, roughly how many total API calls does the loop make?', options: ['1', '3 (2 tool-requesting calls + 1 final answer call)', '0', 'Exactly 2, always'], answerIndex: 1, explanation: "Each tool-calling response triggers another call; the final call (with both results available) produces the answer." },
    ...
    { id: 'c9q10', kind: 'concept', prompt: 'Why is this generalized loop described as the "real shape" of an agent?', options: ['It’s just one implementation detail among many equally valid ones', 'Call -> check -> act -> repeat until done IS the core pattern every capability in this course builds around, unchanged across providers', 'Agents don’t actually use loops', 'It only applies to Compass specifically'], answerIndex: 1, explanation: "This call/check/act/repeat structure is the foundational pattern behind agent behaviour generally, not just this course's implementation." },
  ],
  ...
  homework: {
    ...
    previousHomeworkHint: {
      forLessonNumber: 8,
      hint: "Lesson 8 asked you to implement run_word_count(text) for real and wire it into the (at that point, single-round-trip) tool-calling branch.",
      steps: [
        "Write def run_word_count(text: str) -> str: return str(len(text.strip().split())).",
        "In the dispatch branch, add a check for name == 'get_word_count', calling run_word_count with the text argument.",

...
export const lesson16: StructuredLesson = {
  ...
  concept: {
    ...
    sections: [
      ...
      },
      {
        ...
        body:
          "The voyageai package gives you a client very similar in shape to the anthropic one — install it, create a client, and call .embed() with your text and a model name.",
        code: {
          language: 'python',
          code:
            "import voyageai\n\nvoyage = voyageai.Client()   # reads VOYAGE_API_KEY from the environment\n\ndef get_embedding(text: str) -> list[float]:\n    result = voyage.embed([text], model=\"voyage-3.5\", input_type=\"document\")\n    return result.embeddings[0]",
        },
      },
      {
    ...
    keyTerms: [
      ...
      { term: 'Voyage AI', definition: "Anthropic's recommended embeddings partner; used here via the voyageai Python package." },
    ...
    commonMistakes: [
      ...
      "Assuming the Anthropic client can generate embeddings directly — it can't; embeddings come from a separate provider like Voyage AI.",
    ...
    takeaways: [
      ...
      "An embedding is a numeric vector representing text meaning, generated here via Voyage AI.",
  ...
  finalProject: {
    ...
    placement:
      "Add these functions near the top of main.py, after your existing imports and TOOLS setup, and install voyageai. This lesson introduces the FUNCTIONS; Lesson 17 wires them into Compass's actual conversation flow (deciding when to save/recall automatically).",
    implementation:
      "load_memories() reads memory.json and gracefully returns an empty list if the file doesn't exist yet (a brand-new user) — the fail-gracefully pattern from Module 1's reliability lessons, applied here. save_memory() appends a new fact with its real embedding (via Voyage AI) and REWRITES the whole file — simple and correct for a beginner-scale memory store (a real production system would use an actual vector database for efficiency at scale). recall_relevant() embeds the new question, scores every saved memory by similarity, sorts descending, and returns just the top few most relevant facts as plain strings — ready to be inserted into a prompt.",
    expectedResult:
  ...
  quiz: [
    ...
    { id: 'c16q2', kind: 'concept', prompt: 'Why does Compass use Voyage AI for embeddings instead of the Anthropic client?', options: ['Anthropic doesn’t have a Python SDK', 'Claude models don’t generate embeddings themselves — Voyage AI is Anthropic’s recommended embeddings partner', 'Voyage AI is required for tool use', 'There is no real reason'], answerIndex: 1, explanation: "Embeddings come from a separate, specialized model/provider, not the chat model itself." },
    ...
    { id: 'c16q5', kind: 'code_reading', prompt: 'What does recall_relevant() return when memories is an empty list?', options: ['None', 'An empty list, immediately, without calling the embedding API unnecessarily', 'A crash', 'A random fact'], answerIndex: 1, explanation: "The early return avoids a wasted embedding call when there's nothing to search yet." },

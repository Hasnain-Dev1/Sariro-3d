...
export const lesson12: StructuredLesson = {
  ...
  concept: {
    ...
    sections: [
      ...
      },
      {
        ...
        body:
          "Compass can act, but it still forgets everything between separate runs of the program — every question starts a brand new messages array. Module 3 (Memory + Context) fixes exactly that: short-term conversation memory within a session, and eventually long-term memory across sessions.",
      },
    ],
    keyTerms: [
    ...
    commonMistakes: [
      "Only logging tool activity to the developer console, leaving the actual user with no visibility into what's happening.",
      ...
      "Forgetting a fallback label for a tool name not in the lookup map.",
    ...
    takeaways: [
      ...
      "A simple name-to-label lookup keeps transparency additions clean.",
  ...
  miniProject: {
    ...
    instructions: [
      "Define a small TOOL_LABELS map for 3 made-up tool names.",
    ...
    code: [
      {
        language: 'typescript',
        filename: 'tool-label-test.ts',
        code:
          "const TOOL_LABELS: Record<string, string> = {\n  send_email: 'Sending email...',\n  check_weather: 'Checking the weather...',\n  book_meeting: 'Booking your meeting...',\n};\n\nfunction announceTool(name: string) {\n  console.log(TOOL_LABELS[name] ?? `Using ${name}...`);\n}\n\nannounceTool('check_weather');\nannounceTool('some_future_tool');",
      },
    ],
    explanation:
      "TOOL_LABELS is a simple lookup object — adding a new tool later means adding one new key, with zero changes to announceTool itself. The ?? (nullish coalescing) fallback handles any tool name NOT in the map gracefully, printing a generic-but-still-readable message instead of undefined or an error. This tiny pattern is exactly what gets applied to Compass's real loop next: a small, isolated addition that doesn't touch the core execution logic.",
    expectedOutput:
      "Prints 'Checking the weather...' for the known tool, then 'Using some_future_tool...' for the unknown one — never a crash or undefined.",
    learned: [
      "How to build a simple, extensible label lookup.",
      "How nullish coalescing provides a safe fallback.",
  ...
  finalProject: {
    ...
    fileLocation: "compass-agent/index.ts (add TOOL_LABELS + announce inside the loop)",
    code: [
      {
        language: 'typescript',
      ...
      {
        language: 'typescript',
        filename: 'index.ts (inside askCompass, right before executing a tool)',
        code:
    ...
    placement:
      "Add TOOL_LABELS near your TOOLS array. In askCompass()'s loop, add the console.log(TOOL_LABELS[...] ?? ...) line immediately BEFORE the await executeTool(...) call — so the announcement appears before the work happens, not after.",
    implementation:
      "The label prints the moment the model DECIDES to use a tool, before executeTool actually runs it — this ordering matters: the user should see 'Searching...' WHILE the search happens, not only after it's already done. The existing detailed [tool] log line stays too, still useful for developer debugging, just now alongside a friendlier announcement layer for the end-user-facing REPL experience. Nothing about the loop's core logic (turn cap, message accumulation, dispatch) changes — this is a pure additive polish pass, exactly matching the concept lesson's principle.",
    expectedResult:
  ...
  quiz: [
    ...
    { id: 'c12q5', kind: 'debug', prompt: 'A newly added tool has no entry in TOOL_LABELS. What happens when it’s used?', options: ['The program crashes', 'A generic fallback message like "Using new_tool..." prints instead', 'Nothing prints at all', 'It throws a TypeScript error at runtime'], answerIndex: 1, explanation: "The ?? fallback ensures a reasonable message even for tools not explicitly labeled." },
    { id: 'c12q6', kind: 'application', prompt: 'Why keep the detailed [tool] log line alongside the new friendly label?', options: ['Redundant, one should be removed', 'The detailed log still serves developer debugging; the label serves the end-user experience — different audiences', 'They must always match exactly', 'The friendly label replaces logging entirely'], answerIndex: 1, explanation: "Both serve legitimate but different purposes and can coexist without conflict." },
    { id: 'c12q7', kind: 'concept', prompt: 'What is Compass still missing at the end of Module 2?', options: ['Tool use', 'The ability to remember a conversation ACROSS separate program runs/sessions', 'Error handling', 'A system prompt'], answerIndex: 1, explanation: "Every run currently starts a brand-new messages array — there's no persistence across sessions yet." },
  ...
  homework: {
    ...
    requirements: [
      "Track an array of tool names used during the current question's loop (reset it at the start of each askCompass call).",
      "After the final answer is determined, if the array is non-empty, log '[used: ...]' joining the tool names.",
    ...
    previousHomeworkHint: {
      forLessonNumber: 11,
      hint: "Lesson 11 asked you to add a rate limit to runWebSearch — rejecting a 6th call within any 60-second window.",
      steps: [
        "Declare const searchCallTimes: number[] = []; at module level.",
        "At the top of runWebSearch, filter out timestamps older than 60 seconds: const recent = searchCallTimes.filter(t => Date.now() - t < 60000);",
        "If recent.length >= 5, return an error string immediately without fetching.",
        "Otherwise, push Date.now() onto searchCallTimes (or recent, reassigned) before proceeding with the real request.",
      ],
      codeGuidance: [
        {
          language: 'typescript',
          filename: 'index.ts',
          code:
            "const searchCallTimes: number[] = [];\n\nasync function runWebSearch(query: string): Promise<string> {\n  const now = Date.now();\n  const recent = searchCallTimes.filter((t) => now - t < 60_000);\n  if (recent.length >= 5) return 'Error: search rate limit reached, please wait a moment.';\n  recent.push(now);\n  searchCallTimes.length = 0;\n  searchCallTimes.push(...recent);\n  // ...existing fetch logic...\n}",
        },
      ],
    },
  },

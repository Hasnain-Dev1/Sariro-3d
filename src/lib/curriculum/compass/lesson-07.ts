...
export const lesson07: StructuredLesson = {
  ...
  concept: {
    durationMin: 15,
    summary:
      "Learn what tool use actually is, how the model decides to call a tool, and how to define a tool's schema in Python so Claude knows how and when to use it.",
    sections: [
      {
      ...
      },
      {
        ...
        body:
          "In Python, a tool definition is just a dictionary with three keys: name (how the model refers to it), description (tells the model WHEN and WHY to use it — this matters more than you'd expect), and input_schema (a JSON Schema describing exactly what arguments it needs).",
        code: {
          language: 'python',
          code:
      ...
      },
      {
        ...
        body:
          "You include your tool definitions in the tools parameter of the API call. The model then decides PER MESSAGE whether a tool would help — it might answer directly, or it might respond with a tool_use content block instead of (or alongside) text.",
        code: {
          language: 'python',
          code:
      ...
      },
      {
        ...
        code: {
          language: 'python',
          code:
            "if response.stop_reason == \"tool_use\":\n    tool_block = next(b for b in response.content if b.type == \"tool_use\")\n    print(\"Model wants to call:\", tool_block.name, \"with\", tool_block.input)",
        },
      },
      {
    ...
    keyTerms: [
      ...
      { term: 'Tool definition', definition: "A dict with name, description, and input_schema describing a callable function to the model." },
    ...
    takeaways: [
      ...
      "A tool definition needs a name, a specific description, and an input_schema — just a plain Python dict.",
  ...
  miniProject: {
    ...
    learned: [
      ...
      "The next(generator, default) idiom for finding an optional match.",
    ],
  },
  ...
  finalProject: {
    ...
    code: [
      ...
      },
      {
        ...
        code:
          "def ask_compass(question: str) -> str:\n    try:\n        response = with_retry(lambda: client.messages.create(\n            model=\"claude-sonnet-5\",\n            max_tokens=400,\n            temperature=0.2,\n            system=SYSTEM_PROMPT,\n            tools=TOOLS,\n            messages=[{\"role\": \"user\", \"content\": question}],\n        ))\n\n        if response.stop_reason == \"tool_use\":\n            tool_block = next(b for b in response.content if b.type == \"tool_use\")\n            print(f\"[tool requested] {tool_block.name}({tool_block.input})\")\n            return \"(I'd like to use a tool here — Lesson 8 wires that up!)\"\n\n        return response.content[0].text\n    except Exception as err:\n        print(f\"[error] Compass could not get a response: {err}\")\n        return \"Sorry, I'm having trouble right now. Please try again in a moment.\"",
      },
    ],
    placement:
      "Add the calculator_tool definition and the TOOLS list near your SYSTEM_PROMPT constant. Update ask_compass() (the non-streaming version) to pass tools=TOOLS and add the stop_reason check shown, right after the retry-wrapped call succeeds.",
    implementation:
      "TOOLS is a small list so future lessons can simply add more tool dicts to it — no other code needs to change to register a new tool. The stop_reason == 'tool_use' check is placed BEFORE the plain-text return, since a tool-use response's content[0] would be the tool_use block, not text — trying to read .text from it would fail. For now, detecting the request and printing it is the whole job; the placeholder message is honest about what's happening rather than pretending to answer.",
    expectedResult:
  ...
  quiz: [
    ...
    { id: 'c7q5', kind: 'debug', prompt: 'A student tries response.content[0].text directly on a tool_use response and gets an AttributeError. Why?', options: ['The API is broken', 'content[0] is a tool_use block in that case, not a text block, and has no .text attribute', 'text is spelled wrong', 'Tools disable text entirely'], answerIndex: 1, explanation: "When the model requests a tool, the relevant content block has a different shape than a plain text reply." },
    { id: 'c7q6', kind: 'application', prompt: 'For a tool needing no arguments, what should input_schema["properties"] be?', options: ['None', 'An empty dict {}', 'Omitted entirely, breaking the schema', 'A list'], answerIndex: 1, explanation: "An empty properties dict with an empty required list is a valid schema describing zero needed inputs." },

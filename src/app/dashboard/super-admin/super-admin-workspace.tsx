...
function SuperAdminDashboardInner({ workspace }: { workspace: WorkspaceKey }) {
  const { user } = useAuth();

  /* Each workspace loads only what it shows. The single page used to fetch
     all of this on every visit, whichever panel the super admin came for. */
  const showTotals = useShows('totals');
  const showEnrolments = useShows('enrollments');
  const showCohorts = useShows('cohorts');
  const showAudit = useShows('audit');
  ...
  const toastFor = useCallback((msg: string, kind?: 'success' | 'error') => setToast({ type: kind || 'success', message: msg }), []);

  // ⌘K and the Today queue open these directly: /dashboard/super-admin/classes?do=book-trial.
  useOpsDo({
    'book-trial': () => setShowBookTrial(true),
    teachers: () => setShowTeacherModal(true),
    eligibility: () => setShowAssignmentModal(true),
    users: () => setShowUserManagement(true),
    earnings: () => setShowEarningsReport(true),
    'assign-teacher': () => setShowAssignTeacher(true),
    'assign-seller': () => setShowAssignSeller(true),
    'change-schedule': () => setShowBatchReschedule(true),
  });

  const loadAll = useCallback(async () => {
    const [s, intents, c, logs, actions] = await Promise.all([
      showTotals || showCohorts ? fetchAdminStats() : Promise.resolve(null),
      showEnrolments ? fetchPendingPurchaseIntents() : Promise.resolve([] as PurchaseIntentRow[]),
      showCohorts ? fetchCohorts(cohortFilter) : Promise.resolve([] as CohortRow[]),
      showAudit || showTotals ? fetchAuditLogs(auditFilter, 50) : Promise.resolve([] as AuditLogRow[]),
      showAudit ? fetchAuditActions() : Promise.resolve([] as string[]),
    ]);
  ...
  }, [cohortFilter, auditFilter, showTotals, showEnrolments, showCohorts, showAudit]);
  ...
  useRealtime({
    ...
    onRefresh: () => { loadAll(); },
    enabled: !!user && (showTotals || showEnrolments || showCohorts || showAudit),
  });
  ...
  /* The jobs done in each workspace, in its header. §11, extended: creating
     and scheduling courses is Admin's job; Super Admin watches the resulting
     numbers. A super-admin who needs to act does it by entering an admin
     account, which is logged — rather than having a second, unlogged path to
     the same operation from a dashboard meant for analytics. */
  const headerActions: Partial<Record<WorkspaceKey, ReactNode>> = {
    classes: (
      <>
        <WorkspaceAction icon={UserCheck} primary onClick={() => setShowBookTrial(true)}>Book a trial</WorkspaceAction>
        <WorkspaceAction icon={Calendar} onClick={() => setShowBatchReschedule(true)}>Change schedule</WorkspaceAction>
        <WorkspaceAction icon={Users} onClick={() => setShowTeacherModal(true)}>Teachers</WorkspaceAction>
        <WorkspaceAction icon={BookOpen} onClick={() => setShowAssignmentModal(true)}>Course eligibility</WorkspaceAction>
      </>
    ),
    people: (
      <>
        <WorkspaceAction icon={UserCheck} primary onClick={() => setShowUserManagement(true)}>Users</WorkspaceAction>
        <WorkspaceAction icon={Users} onClick={() => setShowAssignTeacher(true)}>Assign teacher</WorkspaceAction>
        <WorkspaceAction icon={Users} onClick={() => setShowAssignSeller(true)}>Assign seller</WorkspaceAction>
        <WorkspaceAction icon={LinkIcon} href="/dashboard/super-admin/parents">Parent access</WorkspaceAction>
      </>
    ),
    sales: (
      <>
        <WorkspaceAction icon={UserCheck} primary onClick={() => setShowBookTrial(true)}>Book a trial</WorkspaceAction>
        <WorkspaceAction icon={ScrollText} onClick={() => setShowEarningsReport(true)}>Earnings &amp; sales</WorkspaceAction>
      </>
    ),
    finance: (
      <>
        <WorkspaceAction icon={ScrollText} primary onClick={() => setShowEarningsReport(true)}>Earnings &amp; sales</WorkspaceAction>
        <WorkspaceAction icon={DollarSign} href="/dashboard/super-admin/teacher-pay">Tiers &amp; pay</WorkspaceAction>
      </>
    ),
  };
  ...
  return (
    <section className="relative pt-6 sm:pt-8 pb-16 px-4 sm:px-6 lg:px-10">
      <div className="max-w-6xl mx-auto">
        ...
        {workspace === 'today' ? (
          <>
            {/* What is waiting on you, before anything else. See
        ...
        {/* Oversight before totals. A super-admin's job is to notice what is
            broken, and the stat cards say how big the system is, never
            whether it works. */}
        <OpsSection id="health" bare>
          <SystemHealthPanel />
        </OpsSection>

        <OpsSection id="totals" icon={Crown}>
        ...
        {/* The only two readings of a class nobody started are ₹1,250 apart,
            ...
            taught. */}
        <OpsSection id="decisions" icon={HelpCircle}>
          <UnresolvedClassesPanel />
        </OpsSection>

        {/* §29-30. This exists to be empty. For three days a catch-up is
            ...
            is arranged. */}
        <OpsSection id="catchup-overdue" title="Overdue catch-up sessions" icon={CalendarClock}>
          <CatchUpOverduePanel />
        </OpsSection>

        {/* Each answer here is worth three saleable seats. A trial seat with no
            grade cannot be banded, so the class stops being offered as one to
            JOIN and every new booking opens a fresh one. */}
        <OpsSection id="trial-grades" icon={GraduationCap}>
          <TrialGradesPanel />
        </OpsSection>

        {/* Every trial with its whole chain — child, teacher, class, seller,
            and both write-ups. The one answer that matters, a trial with a
        ...
        <OpsSection id="schedule" bare>
          <BatchScheduleSection onOpen={() => setShowBatchReschedule(true)} />
        </OpsSection>

        {/* Lesson Plan Browser — super admin can view all course syllabuses */}
        <OpsSection id="lesson-plans" bare>
          <LessonPlanBrowserSection />
        </OpsSection>

        <OpsSection id="catalog" icon={BookOpen}>
          <div className="card-3d p-5">
        ...
        <OpsSection id="enrollments" bare>
        ...
        <OpsSection id="low-credits" icon={CoinsIcon}>
        ...
        <OpsSection id="credit-requests" icon={CoinsIcon}>
          <CreditRequestsPanel />
        </OpsSection>

        {/* Credit Adjustment — super admin can add/deduct credits from any student */}
        <OpsSection id="credit-adjust" bare>
          <CreditAdjustmentSection onToast={toastFor} />
        </OpsSection>
        ...
        {/* The funnel says how many got through; Demand says who they were. */}
        <OpsSection id="funnel" bare>
          <AnalyticsPanel />
        </OpsSection>

        <OpsSection id="demand" bare>
          <DemandPanel />
        </OpsSection>

        {/* Lead Pipeline — centralized student/lead management */}
        <OpsSection id="leads" bare>
          <LeadPipeline onToast={toastFor} />
        </OpsSection>

        {/* §9. The same trial write-ups the seller works from. A super-admin
            asking "is the trial funnel working" is asking about exactly these
            two opinions. */}
        <OpsSection id="lead-signals" bare>
          <LeadSignalsPanel />
        </OpsSection>

        <OpsSection id="demo-requests" bare>
          <DemoRequestsSection onToast={toastFor} />
        </OpsSection>
        ...
        {/* §64, §66, §67 — who is drifting, who is slipping, which batch is in
            ...
            produced it. */}
        <OpsSection id="risk" icon={ShieldAlert}>
          <RiskPanel />
        </OpsSection>

        {/* Attempts to move a learner's conversation off Sariro: the thing that,
            left unseen, quietly takes students out of the company with a
            departing teacher. */}
        <OpsSection id="chat-policy" icon={ShieldAlert}>
          <PolicyFlagsPanel />
        </OpsSection>

        <OpsSection id="audit" bare>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            <h2 className="text-lg sm:text-xl font-extrabold text-slate-900 flex items-center gap-2" style={{ fontFamily: 'var(--font-jakarta)' }}>
      ...
      {/* Dialogs. Mounted on every workspace, closed, so any job can be opened
          from anywhere with ?do= — see useOpsDo. */}
      ...
      {/* Change schedule — new days/times, an apply-from date, or a break */}
      <BatchRescheduleModal
        open={showBatchReschedule}
        onClose={() => setShowBatchReschedule(false)}
        onDone={() => { setShowBatchReschedule(false); toastFor('Schedule updated'); }}
      />
...
/* ════════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════════ */
function BatchScheduleSection({ onOpen }: { onOpen: () => void }) {
  return (
...
/**
 * The super-admin dashboard, as one workspace. Rendered by
 * /dashboard/super-admin (Today) and /dashboard/super-admin/<workspace>; the
 * shell (sidebar, Today counts, ⌘K) comes from ./layout.tsx and stays mounted
 * as the super admin moves between them.
 */
export default function SuperAdminWorkspace({ workspace }: { workspace: WorkspaceKey }) {
  return (
    <WorkspaceProvider role="super_admin" workspace={workspace}>
      <SuperAdminDashboardInner workspace={workspace} />
    </WorkspaceProvider>
  );
}

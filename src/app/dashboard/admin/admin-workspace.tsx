...
function AdminDashboardInner({ workspace }: { workspace: WorkspaceKey }) {
  const { user } = useAuth();

  /* Each workspace loads only what it shows. */
  const showTotals = useShows('totals');
  const showIntents = useShows('purchase-intents');
  const showCohorts = useShows('cohorts');
  const showRevenue = useShows('revenue');
  const showMonitoring = useShows('monitoring');
  ...
  // ⌘K and the Today queue open these directly: /dashboard/admin/classes?do=schedule-batch.
  useOpsDo({
    'new-course': () => setShowCreateModal(true),
    'schedule-batch': () => setShowScheduleBatch(true),
    'change-schedule': () => setShowBatchReschedule(true),
    'manage-batches': () => setShowManageBatches(true),
    'book-trial': () => setShowBookTrial(true),
    'manual-enroll': () => setShowManualEnroll(true),
  ...
  const loadAll = useCallback(async () => {
    ...
    const [s, intents, c, rev] = await Promise.all([
      ...
      showIntents ? fetchPendingPurchaseIntents() : Promise.resolve([] as PurchaseIntentRow[]),
      showCohorts ? fetchCohorts(cohortFilter) : Promise.resolve([] as CohortRow[]),
      showRevenue ? fetchRevenueStats() : Promise.resolve(null),
    ]);
    setStats(s);
  ...
  }, [cohortFilter, showTotals, showIntents, showCohorts, showRevenue]);
  ...
  useRealtime({
    ...
    onRefresh: () => { loadAll(); },
    enabled: !!user && (showTotals || showIntents || showCohorts || showRevenue),
  });
  ...
  /* The jobs done in each workspace, in its header — the row of nine buttons
     that used to sit above everything, sorted to where each one is needed. */
  const headerActions: Partial<Record<WorkspaceKey, ReactNode>> = {
    classes: (
      <>
        <WorkspaceAction icon={Plus} primary onClick={() => setShowCreateModal(true)}>New course</WorkspaceAction>
        <WorkspaceAction icon={Clock} onClick={() => setShowScheduleBatch(true)}>Schedule batch</WorkspaceAction>
        <WorkspaceAction icon={CalendarClock} onClick={() => setShowBatchReschedule(true)}>Change schedule</WorkspaceAction>
        <WorkspaceAction icon={Users} onClick={() => setShowManageBatches(true)}>Manage batches</WorkspaceAction>
        <WorkspaceAction icon={CalendarClock} onClick={() => setShowBookTrial(true)}>Book a trial</WorkspaceAction>
      </>
    ),
    people: (
      <>
        ...
        <WorkspaceAction icon={Plus} onClick={() => setShowManualEnroll(true)}>Manual enrol</WorkspaceAction>
      </>
    ),
    sales: (
      <WorkspaceAction icon={CalendarClock} primary onClick={() => setShowBookTrial(true)}>Book a trial</WorkspaceAction>
    ),
  };
  ...
  return (
    <section className="relative pt-6 sm:pt-8 pb-16 px-4 sm:px-6 lg:px-10">
      <div className="max-w-6xl mx-auto">
        ...
        <OpsSection id="totals" icon={Shield}>
        ...
        <OpsSection id="batch-control" bare>
          <BatchControl onToast={(msg, kind) => handleToast(kind ?? 'success', msg)} />
        </OpsSection>
        ...
        <OpsSection id="purchase-intents" bare>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg sm:text-xl font-extrabold text-slate-900 flex items-center gap-2" style={{ fontFamily: 'var(--font-jakarta)' }}>
      ...
      {/* Change schedule — new days/times, apply-from date, or a break. Admins
          had no entry point for this; only super-admin and teacher did. */}
...
/**
 * The admin dashboard, as one workspace. Rendered by /dashboard/admin (Today)
 * and /dashboard/admin/<workspace>; the shell (sidebar, Today counts, ⌘K)
 * comes from ./layout.tsx and stays mounted between them.
 */
export default function AdminWorkspace({ workspace }: { workspace: WorkspaceKey }) {
  return (
    <WorkspaceProvider role="admin" workspace={workspace}>
      <AdminDashboardInner workspace={workspace} />
    </WorkspaceProvider>
  );
}
